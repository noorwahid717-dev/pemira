...
export default function TPSValidation() {
  const navigate = useNavigate();
  const [validationState, setValidationState] = useState('validating'); // validating, valid, error
  const [errorType, setErrorType] = useState(null); // already_voted, expired_qr, not_started, closed
  ...
  const validateVotingEligibility = () => {
    ...
    // Simulate random validation scenarios for demo
    const scenarios = ['valid', 'expired_qr', 'not_started', 'closed'];
    const scenario = scenarios[0]; // Use 'valid' for normal flow
    
    if (scenario === 'valid') {
      setValidationState('valid');
    } else {
      setValidationState('error');
      setErrorType(scenario);
    }
  };

  const handleStartVoting = () => {
    // Store TPS mode in session
        ...
        {validationState === 'validating' && (
          <div className="validation-card">
            ...
            <p className="validation-text">
              Sedang memverifikasi hak suara Anda...
            </p>
        ...
        {validationState === 'error' && errorType === 'expired_qr' && (
          <div className="validation-card error">
            ...
            <p className="error-help">
              Minta panitia menampilkan QR terbaru.
            </p>
            <button 
              className="btn-primary"
              onClick={handleRescan}
